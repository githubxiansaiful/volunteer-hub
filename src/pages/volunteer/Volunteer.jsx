                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-20'>
                                    {allVolunteers.map(singleVol => (
                                        <div key={singleVol._id} className='singleVolPost overflow-hidden rounded-lg p-5 border dark:border-[#f1f1f10a] hover-effect'>
                                            ...
                                            <div className="mb-3">
                                                <h2 className="font-semibold text-xl dark:text-white">{window.innerWidth > 768 ? (singleVol.post_title.length > 22 ? singleVol.post_title.slice(0, 22) + '...' : singleVol.post_title) : singleVol.post_title}</h2>
                                            </div>
                                            <div className="flex justify-center items-center space-x-2 mb-3">
                                                <p className="bg-[#f1f1f1] dark:text-white dark:bg-[#f1f1f10a] w-full py-2 px-2 rounded-lg tooltip" data-tip="Volunteer Category"><i className="fa-solid fa-tag"></i> {singleVol.category}</p>
                                                <p className="bg-[#f1f1f1] dark:text-white dark:bg-[#f1f1f10a] w-full py-2 px-2 rounded-lg tooltip" data-tip="Apply Last Date"><i className="fa-solid fa-clock"></i> {new Date(singleVol.deadline).toLocaleDateString()}</p>
                                            </div>
                                            <div>
                                    ...
                                    {allVolunteers.map(tableVol => (
                                        <div key={tableVol._id} className='overflow-hidden rounded-lg p-5 border hover-effect'>
                                            <div className='grid grid-cols-2 gap-3 items-center'>
                                                ...
                                                <div>
                                                    <div className="mb-3">
                                                        <h2 className="font-semibold text-xl dark:text-white">{window.innerWidth > 768 ? (tableVol.post_title.length > 22 ? tableVol.post_title.slice(0, 22) + '...' : tableVol.post_title) : tableVol.post_title}</h2>
                                                    </div>
                                                    <div className="justify-center items-center space-y-2 mb-3">
                                                        <p className="bg-[#f1f1f1] dark:text-white dark:bg-[#f1f1f10a] w-full py-2 px-2 rounded-lg tooltip" data-tip="Volunteer Category"><i className="fa-solid fa-tag"></i> {tableVol.category}</p>
                                                        <p className="bg-[#f1f1f1] dark:text-white dark:bg-[#f1f1f10a] w-full py-2 px-2 rounded-lg tooltip" data-tip="Apply Last Date"><i className="fa-solid fa-clock"></i> {new Date(tableVol.deadline).toLocaleDateString()}</p>
                                                    </div>
                                                    <div>
